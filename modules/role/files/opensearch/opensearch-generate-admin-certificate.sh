...
#  copy contents of wikiforge-ca.pem
...
openssl genrsa -out $1/wikiforge-ca-key.pem 2048
openssl req -new -x509 -sha256 -key $1/wikiforge-ca-key.pem -subj "/C=US/ST=DC/L=Washington/O=WikiForge LLC" -out $1/wikiforge-ca.pem -days 730
...
openssl req -new -key $1/admin-key.pem -subj "/C=US/ST=DC/L=Washington/O=WikiForge LLC/CN=ADMIN_WIKIFORGE" -out $1/admin.csr
openssl x509 -req -in $1/admin.csr -CA $1/wikiforge-ca.pem -CAkey $1/wikiforge-ca-key.pem -CAcreateserial -sha256 -out $1/admin.pem -days 730
