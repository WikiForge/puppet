    ...
    def generate_letsencrypt_certificate(self):
        if self.wildcard:
            ...
            if self.no_existing_key:
                if self.domain in ['*.wikiforge.net', 'wikiforge.net', '*.inside.wf', 'inside.wf', '*.your.wf', 'your.wf', '*.try.wf', 'try.wf']:
        ...
        if not self.quiet:
            ...
            os.system('git -C /srv/ssl/ssl/ config user.email "sre-management@wikiforge.xyz"')
